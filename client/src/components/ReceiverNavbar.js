  ...
  return (
    <div className="shadow-md sticky top-0 left-0 z-50">
      <div className="md:flex py-6 md:px-10 bg-green-500 justify-between z-50">
        ...
        <div>
          <ul
            className={`md:flex md:items-center left-0 w-full justify-between md:w-auto transition-all duration-400 ease-in absolute md:static md:pb-0 pb-10 md:pl-0 pl-9 text-black ${
              open
                ? "top-14 opacity-100"
                : "top-[-490px] md:opacity-100 opacity-0"
            } md:z-auto z-50 bg-green-500 `}
          >
            {Links.map((link) => (
              <li key={link.name} className="md:ml-8 md:my-0 my-5">
                <a
                  href={link.links}
                  className="font-bold hover:text-white duration-500"
                >
                  {link.name}
                </a>
              </li>
